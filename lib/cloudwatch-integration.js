...
module.exports.init = function(options) {
  ...
  // Credentials are ignored if cloudwatchlogs object passed in
...
module.exports.add = function(log) {
  ...
  function upload() {
    if (uploading || logEvents.length <= 0) return;
    ...
    token(function(err, sequenceToken) {
      ...
      // Upload batches of approx 512k max (1MB is limit)
      var batchsize = 0;
      var batch = [];
      var logEvent;
      do {
        logEvent = logEvents.shift();
        batch.push(logEvent);
        batchsize += logEvent.message.length + 40; // size of batch item
      } while (batchsize < 512 * 1024 && logEvents.length > 0);
  ...
  if (!intervalId) {
    // Check whether events in queue each 500ms
